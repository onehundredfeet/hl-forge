...
class PipelineParamBufferBlock {
	...
	public static function allocate(renderer:Renderer, rootsig:RootSignature, vBytes:Int, fBytes:Int, set:EDescriptorSetSlot, length:Int = 16, depth:Int = 3,
			scratchCount:Int = 1) {
		var hasVertex = vBytes > 0;
		var hasFragment = fBytes > 0;
		...
		DebugTrace.trace('RENDER PIPELINE PARAMETER BLOCK Allocating buffer with ${vBytes} vbytes, ${fBytes} fbytes, ${set} set, ${length} length, ${depth} depth');
		...
		buffer._vbuffers = hasVertex ? [] : null;
		buffer._fbuffers = hasFragment ? [] : null;
		...
		if (hasVertex) {
			...
			vidx = rootsig.getDescriptorIndexFromName(GLSLTranscoder.getVariableBufferName(VERTEX, set));
		}
		if (hasFragment) {
			...
			fd.setUsage(true);
			fd.setDynamic(depth);
			DebugTrace.trace('RENDER PIPELINE PARAMETER BLOCK Creating fragment uniform buffer with ${buffer._fragmentBytes} bytes');

			for (i in 0...length) {
				buffer._fbuffers.push(fd.load(null));
			}

			fidx = rootsig.getDescriptorIndexFromName(GLSLTranscoder.getVariableBufferName(FRAGMENT, set));
		}
		...
		DebugTrace.trace('RENDER PIPELINE ALLOCATING desriptor sets ${setDesc.maxSets} length ${buffer._length} depth ${buffer._depth}');
		buffer._ds = renderer.addDescriptorSet(setDesc);
		DebugTrace.trace('RENDER PIPELINE ALLOCATING desriptor sets done ${buffer._ds}');
	...
	public function beginUpdate():Bool {
		...
		if (_filled == _writeHead)
			throw "Already updated this descriptor";
		return true;
	}
